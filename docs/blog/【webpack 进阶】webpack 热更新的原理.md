...
`HMR`作为一个`Webpack`内置的功能，可以通过`HotModuleReplacementPlugin`或`--hot`开启。我们如何在 `webpack` 中使用这个功能呢？
...
- `Js` 文件，就是本次修改的代码，重新编译打包后的，大致是下面这个样子（已删减一些并格式化过，这里看不懂没关系的，就记住是返回要更新的模块就好了），`webpackHotUpdate` 方法就是用来更新模块的，201 对应的是哪个模块，其他的就是要更新的模块的内容了
...
那么问题来了，我修改了文件，浏览器是怎么知道要更新的呢？整个热更新的流程是怎样的呢？
...
// 实例对象的onmessage属性，用于指定收到服务器数据后的回调函数。可以接受二进制数据，blob 对象或者 Arraybuffer 对象
...
- `Bunble Server`：提供文件在浏览器的访问，可以让我们通过 `http://0.0.0.0:9528/201.c0c8f528401bb723e53c.hot-update.js` 的方式访问 `bundle.js`
...
![](https://upload-images.jianshu.io/upload_images/1784460-b0e588a616042f64.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

## 深入——源码阅读

我们还是以这个图来讲，其中启动阶段图中的 1、2、A、B阶段就不讲解了，主要看热更新阶段主要讲 3、4 和 5 阶段
...
在 client-src/default/index.js 中，回去更新 `hash`，并且在 ok 的时候去进行检查更新 `reloadApp`
...
```
 ...
 接下来我们看看 `client-src/default/utils/reloadApp.js` 中的 `reloadApp`。这里又利用node.js的EventEmitter，发出`webpackHotUpdate` 消息。这里又将更新的事情给回了 `webpack`（为了更好的维护代码，以及职责划分的更明确。）
...
function reloadApp(
  ...
  if (hot) {
    ...
    // 又利用node.js的EventEmitter，发出webpackHotUpdate消息。
...
之后的源码都是 `HotModuleReplacementPlugin` 塞入到 `bundle.js` 中。如果我们仔细看我们的打包后的文件的话，开启热更新之后生成的代码会比不开启多出很多东西（为了更加直观看到，可以将其输出到本地），这些就是帮助 `webpack` 在浏览器端去更新 `bundle.js` 的 `HMR Runtime` 代码

#### `__webpack_require__.hmrM`——加载.hot-update.json

接下来我们分析的就是打包出来的代码了，其中就有 `hotCheck` 中调用了 `__webpack_require__.hmrM`
...
再来看 `__webpack_require__.l`，主要通过 `JSONP` 的方式进行，因为`JSONP`获取的代码可以直接执行。
...
本文介绍了 `webpack` 热更新的简单使用、相关的流程以及原理，希望对大家有所帮助
...
-  [19 | webpack中的热更新及原理分析](https://time.geekbang.org/course/detail/100028901-98391)
